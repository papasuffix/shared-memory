...
auto server::start( ) -> bool
{
    ...
    while ( true )
    {
        WaitForSingleObject( module_event, INFINITE );
        //std::cout << "Module event triggered" << std::endl;
...
auto server::send( const char* data, operations operation ) -> bool
{
    ...
    if ( !SetEvent( interface_event ) )
    {
        ReleaseMutex( mutex );
        std::cerr << "Failed to set interface event" << std::endl;
        return false;
    }
